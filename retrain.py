...
python retrain.py --bottleneck_dir=tf_files/bottlenecks --model_dir=tf_files/models/"${ARCHITECTURE}" --output_graph=tf_files/retrained_graph.pb --output_labels=tf_files/retrained_labels.txt --image_dir=tf_files/flower_photos
...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
  ...
  """
    ...
    sub_dirs = sorted(item for item in sub_dirs if tf.gfile.IsDirectory(item))
...
def run_bottleneck_on_image(
  ...
  """
    ...
    resized_input_values = sess.run(
        decoded_image_tensor, {image_data_tensor: image_data}
    )
    # Then run it through the recognition network.
    bottleneck_values = sess.run(
        bottleneck_tensor, {resized_input_tensor: resized_input_values}
    )
...
def add_input_distortions(
  ...
  """
    ...
    precropped_image = tf.image.resize_bilinear(
        decoded_image_4d, precrop_shape_as_int
    )
    precropped_image_3d = tf.squeeze(precropped_image, squeeze_dims=[0])
    cropped_image = tf.random_crop(
        precropped_image_3d, [input_height, input_width, input_depth]
    )
    if flip_left_right:
...
def add_final_training_ops(
    class_count, final_tensor_name, bottleneck_tensor, bottleneck_tensor_size
):
...
def create_model_info(architecture):
  ...
  """
    ...
    if architecture == "inception_v3":
        ...
        if (
            version_string != "1.0"
            and version_string != "0.75"
            and version_string != "0.50"
            and version_string != "0.25"
        ):
            tf.logging.error(
...
def add_jpeg_decoding(
  ...
  """
    ...
    resized_image = tf.image.resize_bilinear(
        decoded_image_4d, resize_shape_as_int
    )
...
def main(_):
    ...
    graph, bottleneck_tensor, resized_image_tensor = create_model_graph(
        model_info
    )
